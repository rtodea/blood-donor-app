...
function registerRestEndpoints(router, resourceName, modelName, modelFields, ModelObject) {
  // ## API object
      ...
      ModelObject.findOne({_id: params.id })
        ...
        .then((updatedPersistedData) => {
          if (updatedPersistedData) {
            response.json(dbModelToRestModel(updatedPersistedData, modelFields));
          }
        })
        .catch(R.partial(onCurrentModelObjectError, [response]));
    })
...
function registerRoutes(router) {
  ...
  registerRestEndpoints(router, 'model-event', ModelEvent.modelName, ['name'], ModelEvent);
}
